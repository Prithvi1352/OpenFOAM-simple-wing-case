...
def write_ascii_stl(filename, triangles, solid_name="sweptWing"):
...
def generate_swept_wing(
    ...
    n_airfoil=241,
    n_span=81,
    filename="wing_NACA2412_sweep15.stl"
):
    """
    Generate full finite swept wing STL using NACA 2412.

    Coordinate system:
        x = freestream / chordwise direction
    ...
    For a rectangular wing, shifting the full section by |y| tan(sweep)
    gives the desired sweep while preserving chord and airfoil shape.
    """
    ...
    for y in y_stations:
        # Symmetric backward sweep: both tips shift downstream
        x_shift = abs(y) * np.tan(sweep_rad)

        section = []
        for x, z in zip(x_airfoil, z_airfoil):
            section.append(np.array([x + x_shift, y, z]))
    ...
    write_ascii_stl(filename, triangles, solid_name=f"NACA2412_sweep{int(sweep_deg)}")

    tip_shift = (span / 2.0) * np.tan(sweep_rad)

    print(f"STL written      : {filename}")
    print(f"Airfoil          : NACA 2412")
    print(f"Chord            : {chord} m")
    print(f"Span             : {span} m")
    print(f"Sweep            : {sweep_deg} deg")
    print(f"Tip x-shift      : {tip_shift:.6f} m")
    print(f"Approx x range   : 0 to {chord + tip_shift:.6f} m")
